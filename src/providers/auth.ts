...
function getProviderSettings(provider: ExternalProviderId): {
  apiKey?: string
  apiKeyHelper?: string
  baseURL?: string
} | null {
...
export function getProviderCredentials(
  ...
  const apiKey =
    PROVIDER_ENV_KEYS[provider].map(key => process.env[key]).find(Boolean) ??
    providerSettings?.apiKey ??
    getProviderApiKeyHelperCached(providerSettings?.apiKeyHelper) ??
    null
  const baseURL =
    PROVIDER_BASE_URL_KEYS[provider].map(key => process.env[key]).find(Boolean) ??
