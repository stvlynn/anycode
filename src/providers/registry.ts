...
const SWITCHABLE_MODEL_PROVIDERS: ExternalProviderId[] = [
  'anthropic',
  'openai',
  'gemini',
]
...
export function listSwitchableModelProviders(): ExternalProviderId[] {
  return [...SWITCHABLE_MODEL_PROVIDERS]
}

export function isKnownProviderId(provider: string): provider is ExternalProviderId {
  return provider in PROVIDERS
}

export function isSwitchableModelProvider(
  provider: string,
): provider is ExternalProviderId {
  return SWITCHABLE_MODEL_PROVIDERS.includes(provider as ExternalProviderId)
}
...
export function getFastModelForActiveProvider(): string {
  ...
  return isKnownProviderId(activeProvider)
    ? getFastModelForProvider(activeProvider)
    : getFastModelForProvider('anthropic')
}
...
export function getVisibleExternalModelOptions(): Array<{
  ...
  return listSwitchableModelProviders()
...
export async function verifyProviderConnection(
  ...
  if (!isSwitchableModelProvider(provider)) {
    return {
      ok: false,
