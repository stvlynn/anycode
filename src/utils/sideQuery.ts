...
import { executeExternalSideQuery, resolveExternalProviderModel } from '../services/api/providerDispatcher.js'
...
export async function sideQuery(opts: SideQueryOptions): Promise<BetaMessage> {
  ...
  if (resolveExternalProviderModel(model)) {
    return (await executeExternalSideQuery({
      model,
      system,
      ...
      tools: tools as unknown as Array<Record<string, unknown>> | undefined,
      tool_choice: tool_choice as { type?: string; name?: string } | undefined,
