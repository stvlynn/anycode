...
import { getProviderCredentials } from '../providers/auth.js'
import { verifyProviderConnection } from '../providers/registry.js'
import {
...
export function useApiKeyVerification(): ApiKeyVerificationResult {
  ...
  const [status, setStatus] = useState<VerificationStatus>(() => {
    if (activeProvider !== 'anthropic') {
      const credentials = getProviderCredentials(activeProvider)
      return credentials.apiKey ? 'loading' : 'missing'
    }
  ...
  const verify = useCallback(async (): Promise<void> => {
    if (activeProvider !== 'anthropic') {
      const result = await verifyProviderConnection(activeProvider)
      if (result.ok) {
        setStatus('valid')
        setError(null)
        return
      ...
      const credentials = getProviderCredentials(activeProvider)
      setStatus(credentials.apiKey ? 'error' : 'missing')
      if (result.message) {
